...
struct_raw <- readMat("../data/raw/SC/HCP_cortical_DesikanAtlas_SC.mat")
...
```{r}
# 1. Get the variance explained by each principal component
# The sdev component of the prcomp object contains the standard deviations
# We square them to get the variance
pca_variances <- sc_pca$sdev^2

# 2. Calculate the total variance in the dataset
# This is the sum of the variances of all principal components
# (Or the sum of the variances of the original scaled columns)
total_variance <- sum(pca_variances)

# 3. Calculate the percentage of variance explained by each PC
percent_variance <- pca_variances / total_variance

# 4. Calculate the cumulative variance explained
cumulative_variance <- cumsum(percent_variance)

# 5. Extract the variance explained by the top 60 components
# Since rank. = 60, sc_pca only returns the top 60 components
# But prcomp calculates sdev for ALL components unless truncated manually elsewhere?
# Wait, prcomp with rank. returns only the top components in rotation/x,
# but let's check sdev length.
# If sdev length is limited to 60, total_variance calculation above is WRONG
# because we're missing the tail.

# CORRECT APPROACH FOR TRUNCATED PCA:
# We need the Total Variance of the original matrix to be the denominator.

# Calculate Total Variance of the cleaned, scaled data
# Since scale.=TRUE, every column has variance = 1.
# Total Variance = Number of Columns (Features)
n_columns <- ncol(sc_flat_clean)
true_total_variance <- n_columns

# Recalculate percent explained using the TRUE total
# We use the top 60 variances from the model
top_60_variances <- sc_pca$sdev[1:60]^2
variance_explained_top60 <- sum(top_60_variances)

percent_explained_top60 <- variance_explained_top60 / true_total_variance

# Output the result
print(paste("Number of Features:", n_columns))
print(paste("Total Variance (Theoretical):", n_columns))
print(paste("Variance Captured by Top 60 PCs:", round(variance_explained_top60, 2)))
print(paste("Percent Variance Explained by Top 60:", round(percent_explained_top60 * 100, 2), "%"))

# Optional: Scree Plot to verify
plot(head(percent_variance, 60), type = "b", 
     main = "Scree Plot: Variance Explained by Vector PCA",
...
func_raw <- readMat("../data/raw/FC/HCP_cortical_DesikanAtlas_FC.mat")
...
```{r}
# --- FUNCTIONAL CONNECTOME PCA VARIANCE ESTIMATION ---

# Check if the PCA model exists
if (!exists("fc_pca")) {
  stop("Error: 'fc_pca' object not found. Please run the Functional PCA step first.")
}

# 1. Get Variances of the Principal Components
# sdev contains the standard deviations of the principal components
# We square them to get the variance of each PC
fc_pca_variances <- fc_pca$sdev^2

# 2. Calculate Total Variance (Theoretical)
# Since you used scale. = TRUE, every column was forced to have a variance of 1.0.
# Therefore, the Total Variance is simply the number of valid features (columns).
# We use the number of columns in the cleaned data used for PCA
n_features_func <- ncol(fc_flat_final)
total_func_variance <- n_features_func

# 3. Calculate Variance Captured by Top 60 PCs
# We sum the variances of the first 60 components
# (Note: prcomp usually returns all components unless rank. was strict, but we check)
top_60_func_variance <- sum(fc_pca_variances[1:60])

# 4. Calculate Percentage Explained
percent_explained_func <- top_60_func_variance / total_func_variance


print(paste("Number of Functional Features:", n_features_func))
print(paste("Total Variance (Theoretical):", n_features_func))
print(paste("Variance Captured by Top 60 PCs:", round(top_60_func_variance, 2)))
print(paste("Percent Variance Explained by Top 60:", round(percent_explained_func * 100, 2), "%"))

# --- Visual Scree Plot ---
# We calculate the proportion for the top 60 to visualize the drop-off
scree_data_func <- data.frame(
  PC = 1:60,
  Variance = fc_pca_variances[1:60],
  Proportion = fc_pca_variances[1:60] / total_func_variance
)

library(ggplot2)
ggplot(scree_data_func, aes(x = PC, y = Proportion)) +
  geom_line() +
  geom_point() +
  labs(title = "Functional PCA Scree Plot",
       subtitle = "Variance Explained per Component",
...
```{r}
write.csv(my_struct_df, "../data/processed/raw_struct_pca.csv")
write.csv(my_func_df, "../data/processed/raw_func_pca.csv")

pca_df <- my_struct_df %>% 
  inner_join(my_func_df, by="Subject")
write.csv(pca_df, "../data/processed/raw_pca_df.csv", row.names = FALSE)
```
...
for (epoch in 1:num_epochs) {
  ...
  if (epoch %% 1 == 0) { # Check every epoch for smooth plot
    ...
    train_data_mat <- as.matrix(train_tensor)
    test_data_mat <- as.matrix(test_tensor)
    
    train_var_actual <- var(c(train_data_mat))  # Variance of ALL values
    test_var_actual <- var(c(test_data_mat))
    
    # TSS = Variance * (n-1)
    train_tss <- train_var_actual * (nrow(train_data_mat) * ncol(train_data_mat) - 1)
    test_tss <- test_var_actual * (nrow(test_data_mat) * ncol(test_data_mat) - 1)
    ...
    if (epoch %% 10 == 0) {
      train_r2 <- 1 - train_norm_loss
      test_r2 <- 1 - test_norm_loss
      cat(sprintf("Epoch %d | R² Train: %.3f (Loss: %.3f) | R² Test: %.3f (Loss: %.3f)\n", 
                  epoch, train_r2, train_norm_loss, test_r2, test_norm_loss))
    }
  }
}
...
ggplot(history_norm_df, aes(x = Epoch, y = NormalizedLoss, color = Type)) +
  ...
  labs(title = "VAE Learning Curve: Unexplained Variance",
     subtitle = "Lower = Better (More variance captured by latent space)",
     y = "Proportion of Unexplained Variance (1 - R²)") +
  theme_minimal()
...
```{r}
# 1. Get Predictions on Test Set (in Log-Scaled world)
model$eval()
with_no_grad({
  output <- model(test_tensor)
  recon_log_scaled <- as.matrix(output[[1]]) # Prediction (0-1)
  actual_log_scaled <- as.matrix(test_tensor) # Truth (0-1)
})

# 2. Inverse Transform: Go back to Original Fiber Counts
# Step A: Un-normalize (multiply by max_val)
# Step B: Inverse Log (expm1 is the inverse of log1p)
recon_raw  <- expm1(recon_log_scaled * max_val)
actual_raw <- expm1(actual_log_scaled * max_val)

# 3. Calculate "Loss" on Original Scale (Mean Squared Error)
mse_raw <- mean((recon_raw - actual_raw)^2)
rmse_raw <- sqrt(mse_raw)

# 4. Calculate Variance Explained (R-Squared) on Original Scale
# This compares the error to the total variety of fiber counts
total_variance_raw <- var(as.vector(actual_raw))
r2_raw <- 1 - (mse_raw / total_variance_raw)

print(paste("Average Error (RMSE):", round(rmse_raw, 2), "fibers"))
...
with_no_grad({
  output <- model_func(test_tensor_func)
  recon_log_scaled <- as.matrix(output[[1]]) # Prediction (0-1)
  actual_log_scaled <- as.matrix(test_tensor_func) # Truth (0-1)
})

# Inverse Transform: Go back to Original Functional Strength
# Step A: Un-normalize (multiply by max_val_func)
# Step B: Inverse Log (expm1 is the inverse of log1p)
recon_raw  <- expm1(recon_log_scaled * max_val_func)
actual_raw <- expm1(actual_log_scaled * max_val_func)

# Calculate "Loss" on Original Scale (Mean Squared Error)
mse_raw <- mean((recon_raw - actual_raw)^2)
rmse_raw <- sqrt(mse_raw)

# Calculate Variance Explained (R-Squared) on Original Scale
# This compares your error to the total variety of functional strengths
total_variance_raw <- var(as.vector(actual_raw))
r2_raw <- 1 - (mse_raw / total_variance_raw)
...
write.csv(vae_df, "../data/processed/vae_df.csv", row.names = FALSE)
```
